...
import { AlternateJob, AnalysisResult, InterviewFeedback, LearningPathResult, TranscriptMessage } from './types';
...
const parseJsonResponse = <T>(text: string, typeName: string): T => {
    try {
        // The API often returns markdown-style JSON blocks. Clean them up.
...
${jdText}`;
    ...
    const response = await ai.models.generateContent({
        model: 'gemini-2.5-pro', // Use Pro for more nuanced analysis
        contents: prompt,
        config: {
            responseMimeType: 'application/json',
            responseSchema: {
            ...
            thinkingConfig: { thinkingBudget: 8192 } // Allocate tokens for deeper analysis
        }
    });
...
export const getJdFromUrl = async (jobUrl: string): Promise<string> => {
    try {
        ...
        if (!urlResponse.ok) {
            throw new Error(`Failed to fetch URL, status: ${urlResponse.status}`);
        }
        const html = await urlResponse.text();
        const prompt = `From the following HTML content, extract and return only the core text of the job description. Ignore all HTML tags, navigation bars, headers, footers, sidebars, and advertisements. The output should be clean, readable text representing the job role and responsibilities.\n\nHTML:\n${html}`;
        ...
        console.error("Error in getJdFromUrl:", error);
        throw new Error("Could not retrieve or parse the job description from the provided link.");
    }
};
...
export const generateInterviewReport = async (transcript: string): Promise<InterviewFeedback> => {
    ...
    const response = await ai.models.generateContent({
        model: 'gemini-2.5-pro', // Pro for detailed feedback
        contents: prompt,
        config: {
            responseMimeType: 'application/json',
            responseSchema: {
